...
const RoleSelection = () => {
  ...
  const roles = [
    ...
    { label: "Scrum Product Owner", value: "scrum-product-owner" },
  ...
  return (
    <>
      <div className="select-role-wrapper">
        <h2 className="title">Select a Role</h2>
        <div className="role-list">
          {roles.map((role) => (
            <RoleCheckbox
              selected={role.value === selectedOption}
              key={role.value}
              option={role.label}
              handleSelection={() => handleSelection(role.value)}
            />
          ))}
        </div>
        <PopUp
        ...
        />
        <button onClick={validateRole} className="button">
          Continue
        </button>
      </div>
    </>
